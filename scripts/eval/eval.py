...
# Safety net (must run before any CUDA allocation): cap this process's VRAM so a
# runaway allocation raises a catchable torch.cuda.OutOfMemoryError instead of
# crashing the driver/host. Measured cost for this checkpoint is ~15.6GB
# (bf16 weights, no grad); 85% of a 32GB card leaves ample headroom.
if torch.cuda.is_available():
    torch.cuda.set_per_process_memory_fraction(0.85, device=torch.device("cuda:0"))
